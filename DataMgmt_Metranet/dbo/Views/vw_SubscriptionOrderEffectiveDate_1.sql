...
create view [dbo].[vw_SubscriptionOrderEffectiveDate]
as
SELECT 
convert(DATETIME2 ,SRC.OrderEffectiveDate, 101) as OrderEffectiveDate
,subscription.id_sub
FROM rpt_v_Subscription as subscription
inner join 
(
SELECT	sub.id_sub
		,tv.nm_value AS OrderEffectiveDate
		...
		WHERE tv.c_spec_name = 'OEDate' and nullif(tv.nm_value,'') is not null and tv.c_start_date >= sub.vt_start 
)  AS SRC on subscription.id_sub = SRC.id_sub 
where RowRank = 1
group by subscription.id_sub
,SRC.OrderEffectiveDate