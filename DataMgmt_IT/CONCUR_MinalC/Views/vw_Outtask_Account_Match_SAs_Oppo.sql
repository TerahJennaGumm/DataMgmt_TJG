create view  [CONCUR\MinalC].[vw_Outtask_Account_Match_SAs_Oppo]
as (
SELECT
-- vw.*
distinct 
...
FROM 
dm_cons.[dbo].[vw_Outtask_Company_List_SA] sa 
left join metranet_data.dbo.t_account_mapper map
on map.nm_login = sa.CONTRACT_EXTERNAL_REFERENCE_ID
left join metranet_data.dbo.rpt_v_accountflattenedhierarchy h
on map.id_acc = h.service_id_acc 
left join metranet_data.dbo.rpt_v_account acc
on ( h.business_id_acc = acc.id_acc )
left join metranet_data.dbo.rpt_v_account acc1
on (  h.secondary_business_id_acc = acc1.id_acc )
left join salesforce_data.dbo.account a
