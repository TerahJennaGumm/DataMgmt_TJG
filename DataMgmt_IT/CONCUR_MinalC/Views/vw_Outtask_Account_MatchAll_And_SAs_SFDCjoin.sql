...
CREATE view [CONCUR\MinalC].[vw_Outtask_Account_MatchAll_And_SAs_SFDCjoin]
 as (
select 
...
from [CONCUR\MinalC].[vw_Outtask_Account_MatchAll_And_SAs_join] s
left join salesforce_data.dbo.Account a
on a.id = SAMatch_MN_Bus_Login
left join salesforce_data.dbo.Account a1
on a1.id =  s.id
left join DM_CONS.dbo.Outtask_Company_List l
on l.company_id = company_id_SAMatch
left join salesforce_data.dbo.Account a2
