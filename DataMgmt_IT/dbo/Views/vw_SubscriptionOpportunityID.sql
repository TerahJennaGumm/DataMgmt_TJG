...
,subscription.id_sub
FROM [DataMgmt_Metranet].dbo.rpt_v_Subscription as subscription
inner join 
(
SELECT	sub.id_sub
		,tv.nm_value AS SFDCOpportunityID
		,ROW_NUMBER() OVER (PARTITION BY sub.id_sub order by tv.c_start_date desc ) as RowRank
		from [DataMgmt_Metranet].dbo.rpt_v_Subscription AS sub 
		INNER JOIN  [DataMgmt_Metranet].dbo.t_char_values AS tv on sub.id_sub = tv.id_entity
