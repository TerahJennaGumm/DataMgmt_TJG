  ...
  CREATE VIEW [dbo].[vw_SFDC_Active_Direct_Indirect_Customers_SFDC_Metranet_Status] AS 
  (
...
LEFT JOIN [DataMgmt_Metranet].[dbo].[vw_Metranet_BusinessCustomer_Status] ss ON [MNAccountName] = a.id
WHERE 
  (a.type = 'Customer'    AND site = 'Billing Account' ) OR ( a.type = 'Indirect Customer')
   )
