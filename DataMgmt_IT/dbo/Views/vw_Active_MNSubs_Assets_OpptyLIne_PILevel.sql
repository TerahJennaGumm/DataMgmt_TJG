...
create view [dbo].[vw_Active_MNSubs_Assets_OpptyLIne_PILevel] as
SELECT 
...
FULL outer join [$(DataMgmt_SFDC)].dbo.asset ass on ((ass.Opportunity__c = LEFt(f.nm_value,18) and a.id_po = MN_PO_ID__c))
LEFT OUTER JOIN [$(DataMgmt_SFDC)].dbo.Opportunity o ON ( LEFt(f.nm_value,18) = o.Id ) --OR (ass.Opportunity__c = o.Id ) 
left outer join [$(DataMgmt_SFDC)].dbo.OpportunityLineItem li ON (o.Id = li.OpportunityId AND li.MN_PO_ID__c = a.id_po and li.MN_PI_ID__c = ass.MN_PI_ID__c)
